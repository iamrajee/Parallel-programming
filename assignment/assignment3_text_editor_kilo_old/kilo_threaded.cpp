...
clock_t Thread_time = clock(); //starting clock
using namespace std;
    ...
    switch (c) {
        ...
        case CTRL_KEY('q'):
            if (E.dirty && quit_times > 0) {
                ...
                quit_times--;
                //cout << "\033[1;4;30m" << "\n\n \033[1;4;32m TOTAL_CPU TME = "<<(double)(clock() - tStart)/CLOCKS_PER_SEC <<"\033[0m ";
                cout << "\033[1;4;30m" << "\n\n \033[1;4;32m "<<"************* Thread_time = "<<(double)(Thread_time)/CLOCKS_PER_SEC << " Sec, "<<" TOTAL_CPU TME = "<<(double)(clock() - tStart)/CLOCKS_PER_SEC << " Sec *********"<<"\033[0m ";
                return;
            }
            write(STDOUT_FILENO, "\x1b[2J", 4);
            write(STDOUT_FILENO, "\x1b[H", 3);
...
/**************************** INSTEAD OF DOING SIMPLY LIKE THIS I'M LIKE THREAD VERSION *******************/
    //enableRawMode();
    // initEditor();
    // if (argc >= 2){
    //     editorOpen(argv[1]);
...
/********************************** THREAD VERSION **********************************************/
    Thread_time = clock();
    pthread_t enableRawMode_thread,initEditor_thread,editorOpen_thread;
    int  ret;

    ret = pthread_create( &enableRawMode_thread, NULL, enableRawMode, NULL);
    if(ret) printerrormsg(ret);                    //if return value equal to 1, then print error message
    cout << "\033[1;4;30m"<<"enableRawMode_thread created!"<<"\033[0m\n"<<endl;

    ret = pthread_create( &initEditor_thread, NULL, initEditor, NULL);
    if(ret) printerrormsg(ret);                    //if return value equal to 1, then print error message
    cout << "\033[1;4;30m"<<"initEditor_thread created!"<<"\033[0m\n"<<endl;
    
    if (argc >= 2){
      ret = pthread_create( &editorOpen_thread, NULL, editorOpen, (void *) argv[1]);
      if(ret) printerrormsg(ret);                    //if return value equal to 1, then print error message
      cout << "\033[1;4;30m"<<"editorOpen_thread created!"<<"\033[0m\n"<<endl;
      pthread_join( editorOpen_thread, NULL);
    }

    /*Waiting for all thread to end*/
    pthread_join( enableRawMode_thread, NULL);
    pthread_join( initEditor_thread, NULL);
